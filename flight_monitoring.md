...
| 일반 현대카드 | 1,385,500원 |
| 일반결제 | 1,386,100원 |
| 수하물 | 기내 1개 + 개인 물품 1개, 합산 12kg 이하 · 위탁수하물 불포함 · 23kg 1개 사전 구매 예상 왕복 200,000~280,000원 · 수하물 포함 예상 총액 1,582,900~1,662,900원 |
| 예약 | [선택 항공편 상세 열기](https://flight.naver.com/flights/international/detail/SEL:city-LIS:airport-20260903/LIS:airport-SEL:city-20260916?adult=1&isDirect=false&fareType=Y&selectType=concurrent&selectedFlight=1%3A20260903ICNAMSKL0856%2B20260904AMSLISKL1583%3AA01%2FB72%3AHK%3AKL%3A%2C2%3A20260916LISCDGAF1225%2B20260916CDGICNAF0264%3AA01%2FB72%3AHK%3AKL%3A) |
...
- 노선: 서울(ICN/GMP) ↔ 리스본(LIS) 왕복, 성인 1명, 일반석 · 포르투 출발 오픈조는 조회하지 않음
- 날짜: 2026년 9월 1~4일 한국 출발, 9월 13~18일 리스본 출발
...
- 섹션 순서는 `리스본 왕복 직항 최저가 항공권` → `리스본 왕복 경유 조건 충족 항공권` → `이전 최저가 이력` → `조회 조건` → `출력 형식`
- 직항은 최저가 1개만, 경유는 조건 충족 항공권을 모두 가격순으로 번호를 붙여 표시
- 항공권마다 `항목 | 내용` 2열 세로 표를 사용하고 HTML 줄바꿈 태그와 여행사명은 사용하지 않음
- 여행일, 항공사, 편명, 출발·도착시각, 총 비행시간, 경유지와 대기시간, 결제수단별 가격, 수하물과 선택된 네이버 상세 링크를 표시
